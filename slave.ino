...
// FIX
//
//savedata + variable save from canbuss
//
//
//------------------------------------------------------------------------------------------------
...
// const byte _Address = 120;
// const byte _Address = 128;
...
const int LCD_Log_Interval = 500;
unsigned long LCD_Last_Log_Time = 0;

const int T1_Interval = 100;
...
bool cutoff_act = 0;                // Cutoff activated (treshold reached)?

// TPS
int tps = 0;                       // 0-100% TPS Sensor reading (Canbus)
...
void setup() {
  ...
  // MAP
  pinMode(mpx4250dp, INPUT);                           // Configuring pin for /INT input

  // PWM
  pinMode(PWM_OUT, OUTPUT);                            // Configuring pin for /INT output
  ...
  // Start serial
  Serial.begin(115200);
  Serial.setTimeout(10000); // 10sek timoeout
...
void loop() {
  ...
  // Sensordata
  if ((millis() - Last_T1_Time) > T1_Interval) {
    Last_T1_Time = millis();

    Get_EGT();                  // Get egt reading
    Get_Boost();                // Get map reading
    canbuss_send_Sensordata();  // Send sensordata in timly manner (without request)
...
// ************************************************
// Canbuss Send Sensordata
//
// ************************************************

void canbuss_send_Sensordata() {

  int vala;
  int valb;

  //mapsensor(kpa) A(0-255)
  canbuss_send(0x1F, 1, 1, mapsensor, 0x00, 0x00, 0x00, 0x00, 0x00);

  // egtsensor(celcius)  data=((A*8)+B)
...
void Boostcontrol() {
  ...
  if (cutoff_act == 1) {
    ...
    if (use_static_boost == 1) {
      ...
      // Constra in and set minimum boost
  ...
  if (Serial.available() > 0) {
    ...
    if (recieved == '\n') {
      ...
      if (S_inData == "h\n") {
        ...
        Serial.println(F("sbc   Boost controller"));
