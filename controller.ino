...
void canbuss_read() {
  ...
  if (!digitalRead(CAN0_INT)) {              // If CAN0_INT pin is low, read receive buffer
    ...
    // if ((canbuss_rxId == 2015) && (canbuss_in[2] == 11)) {    //Check if remote id is HEX 7DF
    // Serial.println(F(" > Canbuss Req map "));
    //      canbuss_send(0x8DF, 0x03, 0x41, 0x0b, mapsensor, 0x00, 0x00, 0x00, 0x00);
    // }


    if (canbuss_rxId == 31)     //Check if remote id is HEX 1F (Push data)
    {
      //mapsensor
      if ((canbuss_in[0] == 1) && (canbuss_in[1] == 1)) mapsensor = canbuss_in[2];

      //egtsensor
      if ((canbuss_in[0] == 2) && (canbuss_in[1] == 1)) egtsensor = (canbuss_in[2] * 8) + canbuss_in[3] ;

      //boost_wanted
      if ((canbuss_in[0] == 10) && (canbuss_in[1] == 1)) boost_wanted = canbuss_in[2];

      //cutoff_act
      if ((canbuss_in[0] == 100) && (canbuss_in[1] == 1)) cutoff_act = canbuss_in[2];
    }
